...
for ii = 1:rounds
	...
	for current_agent = 1:no_agents
		...
		while sum(M_k(:, current_agent)) > 0							%Solange es noch unbesuchte StÃ¤dte gibt
			...
			city_s = choose_city(tau,beta0, M_k(:,current_agent), current_city, no_cities, current_agent, q0);              %Wähle eine Stadt
		...
		tau(start_city, current_city) = (1-alpha)*tau(start_city, current_city)+alpha*tau0;									%Pheromaninupdate für nachhause weg
