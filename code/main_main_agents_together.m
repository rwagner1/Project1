...
for ii = 1:rounds
    	...
    	for current_agent = 1:no_agents
            ...
            if (jj == no_cities)
                ...
                if current_city(current_agent) < start_city(current_agent)
                    ...
                    tau(current_city(current_agent), start_city(current_agent)) = (1-alpha)*tau(current_city(current_agent), start_city(current_agent))+alpha*tau0;         %Lokales Pheromenupdate für heimweg
                                
                else
                                
                    path_length(current_agent) = path_length(current_agent) + data_set(start_city(current_agent), current_city(current_agent));
                    tau(start_city(current_agent), current_city(current_agent)) = (1-alpha)*tau(start_city(current_agent), current_city(current_agent))+alpha*tau0;         %Lokales Pheromenupdate für heimweg
    ...
    for ll = 2:no_agents
        ...
        if (path_length(ll) < shortest_path)
            ...
            shortest_path_index = ll;                                   %Index zum Agent mit dem kürzesten Pfad
